...
export function SignUpForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Sign in via NextAuth so session cookie is set
    ...
    if (result?.error) {
      // Account created but couldn't sign in immediately — email confirmation may be required
  ...
  return (
    <div className="w-full max-w-md">
      <div className="card p-8">
        <h1 className="font-display text-2xl italic text-brand-brown-deep mb-1">Create account</h1>
        <p className="text-sm text-brand-text-secondary mb-6">Join us for orders, custom cakes, and more</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="fullName" className="block text-xs text-brand-text-secondary mb-1.5">Full name</label>
            <input
              id="fullName"
              type="text"
              value={fullName}
          ...
          <div>
            <label htmlFor="password" className="block text-xs text-brand-text-secondary mb-1.5">Password</label>
            <input
              id="password"
              type="password"
              value={password}
      ...
      <p className="mt-4 text-center text-sm text-brand-text-secondary">
        ...
        <Link href="/auth/signin" className="text-brand-gold hover:underline font-medium">
          Sign in
        </Link>
      </p>
    </div>
