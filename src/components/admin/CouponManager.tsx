...
type FormState = {
  ...
  min_order_value: string; max_uses: string; expires_at: string; is_active: boolean
}

const EMPTY: FormState = {
  code: '', discount_type: 'percentage', discount_value: 10,
  min_order_value: '', max_uses: '', expires_at: '', is_active: true,
}
...
export function CouponManager({ initialCoupons }: Props) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setSaving(false)
    if (!res.ok) { const { error: err } = await res.json(); setError(err ?? 'Save failed'); return }
  ...
  return (
    <div className="space-y-4">
      {/* Create form */}
      {showForm && (
        <div className="rounded-2xl border p-5" style={{ background: 'rgba(255,255,255,0.02)', borderColor: 'rgba(200,151,58,0.12)' }}>
          <p className="text-sm font-medium text-brand-cream mb-4">New Coupon</p>
          <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-3">
            <div>
              <label className={labelClass}>Code *</label>
              <input value={form.code} required className={inputClass} style={inputStyle} placeholder="WELCOME20"
            ...
            <div>
              ...
              <select value={form.discount_type} onChange={(e) => setForm((f) => ({ ...f, discount_type: e.target.value as 'percentage' | 'flat' }))}
                ...
                <option value="percentage" style={{ background: '#1a0f07' }}>Percentage (%)</option>
                <option value="flat" style={{ background: '#1a0f07' }}>Flat (₹)</option>
              </select>
            </div>
            <div>
              <label className={labelClass}>Value *</label>
              <input type="number" value={form.discount_value} min={1} required className={inputClass} style={inputStyle}
                onChange={(e) => setForm((f) => ({ ...f, discount_value: Number(e.target.value) }))} />
            </div>
            <div>
              <label className={labelClass}>Min order (₹)</label>
              <input type="number" value={form.min_order_value} min={0} className={inputClass} style={inputStyle} placeholder="None"
                onChange={(e) => setForm((f) => ({ ...f, min_order_value: e.target.value }))} />
            </div>
            <div>
              <label className={labelClass}>Max uses</label>
              <input type="number" value={form.max_uses} min={1} className={inputClass} style={inputStyle} placeholder="Unlimited"
                onChange={(e) => setForm((f) => ({ ...f, max_uses: e.target.value }))} />
            </div>
            <div>
              <label className={labelClass}>Expires at</label>
              <input type="date" value={form.expires_at} className={inputClass} style={inputStyle}
      ...
      {/* Table */}
      <div className="rounded-2xl border overflow-x-auto" style={{ borderColor: 'rgba(200,151,58,0.08)' }}>
        <table className="w-full text-sm min-w-[640px]">
          ...
          <tbody className="divide-y" style={{ borderColor: 'rgba(200,151,58,0.06)' }}>
            {coupons.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-brand-gold/40">No coupons yet.</td></tr>
            )}
            {coupons.map((c) => (
              <tr key={c.id} className="hover:bg-white/5 transition-colors">
                ...
                <td className="px-4 py-3 text-xs text-brand-gold/60">{c.min_order_value ? `₹${c.min_order_value}` : '—'}</td>
                ...
                <td className="px-4 py-3">
                  <button onClick={() => handleDelete(c.id)} className="text-xs text-red-400/40 hover:text-red-400 transition-colors">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
