        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y" style={{ borderColor: 'rgba(200,151,58,0.06)' }}>
            {categories.length === 0 && !isEditing('new') && (
              <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-sm text-brand-gold/40">
                  No categories yet.
                </td>
              </tr>
            )}
            {categories.map((cat) =>
              isEditing(cat.id) ? (
                <tr key={cat.id} style={{ background: 'rgba(200,151,58,0.04)' }}>
                  ...
                  <td className="px-4 py-3">
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      >
                        <div
                          className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-transform ${form.is_active ? 'translate-x-4' : 'translate-x-0.5'}`}
                        />
                      </div>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={cancelEdit}
                        className="px-3 py-1.5 rounded-lg text-xs text-brand-gold/50 hover:text-brand-gold border transition-colors"
                ...
                <tr key={cat.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${cat.is_active ? 'bg-green-900/30 text-green-400' : 'bg-red-900/20 text-red-400'}`}
                    >
              ...
              <tr style={{ background: 'rgba(200,151,58,0.04)' }}>
                <td className="px-4 py-3">
                  <input
                    ...
                    autoFocus
                    onChange={(e) =>
                      setForm((f) => ({ ...f, name: e.target.value, slug: toSlug(e.target.value) }))
                    }
                  />
                </td>
                <td className="px-4 py-3">
