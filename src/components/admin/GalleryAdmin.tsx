...
export function GalleryAdmin({ initialImages }: Props) {
  ...
  async function move(id: string, direction: 'up' | 'down') {
    ...
    await fetch('/api/admin/gallery/reorder', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(
        reordered.map(({ id: imgId, sort_order }) => ({ id: imgId, sort_order }))
      ),
    })
  }
      ...
      {images.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {images.map((img, idx) => (
            <div
              key={img.id}
              className={`rounded-2xl overflow-hidden border group relative aspect-square ${!img.is_active ? 'opacity-50' : ''}`}
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-3">
                <div className="flex justify-between">
                  <button
                    ...
                    disabled={idx === 0}
                    className="text-white/70 hover:text-white disabled:opacity-20 text-lg leading-none"
                  >
                    ‹
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="text-white/70 hover:text-white disabled:opacity-20 text-lg leading-none rotate-0"
                  >
                    ›
                  </button>
                </div>
                  ...
                  {editingAlt === img.id ? (
                    <div className="flex gap-1">
                      ...
                      <button
                        ...
                        className="text-xs text-brand-gold px-2"
                      >
                        ✓
                      </button>
                    </div>
