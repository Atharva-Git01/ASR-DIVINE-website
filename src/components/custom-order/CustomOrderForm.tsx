...
const ORDER_TYPES = [
  {
    ...
    icon: '🎂',
    desc: 'Custom designed for birthdays, weddings, and special events',
  },
  {
    id: 'chocolates',
    label: 'Chocolate Box',
    icon: '🍫',
    desc: 'Personalised assortment — flavours, packaging, and message',
  },
  {
    id: 'hamper',
    label: 'Gift Hamper',
    icon: '🎁',
    desc: 'Curated combination of chocolates, cookies, and treats',
  },
  {
    id: 'cookies',
    label: 'Custom Cookies',
    icon: '🍪',
    desc: 'Shaped, iced, or branded for corporate and personal gifting',
  },
]
    ...
    return (
      <div className="text-center py-10 space-y-4">
        ...
        <p className="text-sm text-brand-text-secondary max-w-sm mx-auto">
          Thanks {form.name}! We&apos;ll review your request and get back to you at{' '}
    ...
    <div>
          ...
          return (
            <div key={s} className="flex items-center flex-1">
              <div className="flex flex-col items-center">
                ...
                <span
                  className={`mt-1 text-[10px] ${active ? 'text-brand-brown-deep font-medium' : 'text-brand-text-secondary'}`}
                >
                  {STEP_LABELS[s]}
                </span>
              </div>
              {i < 3 && (
                <div
                  ...
                  style={{
                    background: stepIndex > i ? 'var(--color-brown-deep)' : 'rgba(44,26,14,0.10)',
                  }}
                />
              )}
            </div>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className="input"
              placeholder="Chocolate, Red Velvet, Mixed…"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => set('designDescription', e.target.value)}
              rows={4}
              required
              className="input resize-none"
              placeholder="Describe what you have in mind — colours, style, personalisation, text…"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="input"
              placeholder="Eggless, nut-free, sugar-free…"
            />
          </div>
          ...
          <div>
            ...
            {form.referenceImages.length > 0 && (
              <p className="mt-1 text-xs text-brand-text-secondary">
                {form.referenceImages.length} file(s) selected
              </p>
            )}
          </div>

          <div className="flex gap-3">
            <button onClick={() => setStep('type')} className="btn-ghost">
              ← Back
            </button>
            <button
              disabled={!form.designDescription}
      ...
      {step === 'delivery' && (
        <div className="space-y-5">
          ...
          <div>
            ...
            />
            <p className="mt-1 text-xs text-brand-text-secondary">
              Minimum 5 business days. For elaborate designs, allow 10+ days.
            </p>
          </div>
            ...
            <div className="flex gap-3">
              {['delivery', 'pickup'].map((type) => (
                <button
                  ...
                  style={
                    form.deliveryType !== type ? { borderColor: 'rgba(44,26,14,0.20)' } : undefined
                  }
                >
      ...
      {step === 'confirm' && (
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Summary */}
          <div
            className="card p-4 text-sm space-y-1.5"
            style={{ background: 'rgba(44,26,14,0.02)' }}
          >
            ...
            <p>
              ...
              {ORDER_TYPES.find((t) => t.id === form.orderType)?.label}
            </p>
            {form.occasion && (
              <p>
                <span className="text-brand-text-secondary">Occasion:</span> {form.occasion}
              </p>
            )}
            <p>
            ...
            <p>
              <span className="text-brand-text-secondary">Fulfillment:</span>{' '}
              {form.deliveryType === 'delivery' ? 'Delivery' : 'Studio pickup'}
            </p>
            {form.budget && (
              <p>
                <span className="text-brand-text-secondary">Budget:</span>{' '}
                {form.budget.replace(/-/g, ' ')}
              </p>
            )}
          </div>
