        ...
        <div className="space-y-3">
          {messages.map((m) => (
            <div
              ...
              style={{
                background: m.is_read ? 'rgba(255,255,255,0.02)' : 'rgba(200,151,58,0.04)',
                borderColor: m.is_read ? 'rgba(200,151,58,0.08)' : 'rgba(200,151,58,0.16)',
              }}
            >
  ...
  return (
    <form action={`/api/admin/messages/${id}/read`} method="POST" className="flex-shrink-0">
      {!isRead && (
        <button
          type="submit"
          className="text-xs text-brand-gold/50 hover:text-brand-gold transition-colors whitespace-nowrap"
        >
          Mark read
        </button>
      )}
