...
async function getDashboardData() {
  ...
  const [
    ...
    db.from('orders').select('*', { count: 'exact', head: true }).gte('created_at', `${today}T00:00:00`),
    db.from('orders').select('id, order_number, status, total, created_at').order('created_at', { ascending: false }).limit(8),
    db.from('orders').select('created_at, total').eq('payment_status', 'paid').gte('created_at', weekAgo).order('created_at'),
    db.from('order_items').select('product_name, quantity').order('quantity', { ascending: false }).limit(5),
  ])
  ...
  return { totalRevenue, totalOrders: totalOrders ?? 0, pendingOrders: pendingOrders ?? 0, todayOrders: todayOrders ?? 0, recentOrders: recentOrders ?? [], last7Days, maxRev, topList }
}

export default async function AdminDashboard() {
  const { totalRevenue, totalOrders, pendingOrders, todayOrders, recentOrders, last7Days, maxRev, topList } =
    await getDashboardData().catch(() => ({
      totalRevenue: 0, totalOrders: 0, pendingOrders: 0, todayOrders: 0,
      recentOrders: [], last7Days: [], maxRev: 1, topList: [],
    }))
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-brand-gold/50 mt-0.5">Welcome back. Here&apos;s what&apos;s happening today.</p>
      </div>

      {/* Stats */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total Revenue" value={`₹${totalRevenue.toLocaleString('en-IN')}`} sub="All paid orders" accent />
            ...
            {last7Days.map(({ day, rev }) => {
              ...
              return (
                <div key={day} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <div className="w-full rounded-t-md transition-all" style={{ height: `${Math.max(h, 2)}%`, background: rev > 0 ? 'var(--color-gold)' : 'rgba(200,151,58,0.15)', minHeight: '4px' }} />
              ...
              {topList.map(([name, qty], i) => {
                ...
                return (
                  <div key={name}>
                    ...
                    <div className="h-1.5 rounded-full" style={{ background: 'rgba(200,151,58,0.15)' }}>
                      <div className="h-full rounded-full" style={{ width: `${(qty / maxQty) * 100}%`, background: i === 0 ? 'var(--color-gold)' : 'rgba(200,151,58,0.5)' }} />
                    </div>
                  </div>
                )
              })}
      ...
      {/* Recent orders */}
      <div className="rounded-2xl border" style={{ background: 'rgba(255,255,255,0.03)', borderColor: 'rgba(200,151,58,0.08)' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: 'rgba(200,151,58,0.08)' }}>
          <h2 className="text-sm font-medium text-brand-cream">Recent orders</h2>
          <Link href="/admin/orders" className="text-xs text-brand-gold/60 hover:text-brand-gold transition-colors">View all →</Link>
        </div>
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <span
                    ...
                    style={{ background: `${STATUS_COLOR[order.status] ?? '#8B5E3C'}18`, color: STATUS_COLOR[order.status] ?? '#8B5E3C' }}
                  >
                    {order.status.replace(/_/g, ' ')}
                  </span>
                  <span className="text-sm font-medium text-brand-gold">₹{order.total?.toLocaleString('en-IN')}</span>
                </div>
              </Link>
            ))}
          </div>
